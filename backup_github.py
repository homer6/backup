...
def main():
    # --- Argument Parsing ---
    parser = argparse.ArgumentParser(description="Backup all GitHub repositories for an organization to a local directory and optionally upload to S3.")
    ...
    # Archive options
    parser.add_argument("--create-archives", action="store_true", help="Create archives of each repository.")
    parser.add_argument("--volume-size", default="1G", help="Size limit for each archive volume (e.g., '1G', '500M'). Default is 1G.")
    
    # S3 upload options
    parser.add_argument("--upload-to-s3", action="store_true", help="Upload archives to S3 bucket.")
    parser.add_argument("--s3-profile", default="", help="AWS profile for S3 upload.")
    parser.add_argument("--s3-bucket", default="", help="Destination S3 bucket name.")
    parser.add_argument("--s3-path", default="", help="Destination path within the bucket (default: github_backups/<org_name>).")
